				     ...
				     TEE_Param params[TEE_NUM_PARAMS])
{
	...
	IMSG("This is the cntpct: %lu", read_cntpct());
	...
	user_mode_ctx_print_mappings(uctx);
	IMSG("This is experimental ptr sp %p", thread_get_usr_sp());
	void* *fp;
	...
	IMSG("FP is now in attestation.c: %p", fp);
	IMSG("FP2 is now in attestation.c: %p", fpInSysMode);
	IMSG("SP is now of attestation.c: %p", sp);


	IMSG("---------------------------------");
	IMSG("In cmd hash memory");
	void* *stackptr = (void*) thread_get_usr_sp();
	IMSG("This is experimental ptr %p", stackptr);
	asm volatile ("mov %0, fp\n\t"
    	 : "=r" (fp)
    	);
...
// Offset to TA memory: 0x117000
void* *valid_addresses[] = {0x117348, 0x1176a0, 0x117ac8};  
   
// Loop until our FP points to 0x0;
while(*(fp-1) != 0) {
  ...
  int notFound = 1;
  	for(int i = 0; i < 3; i++) {
    		...
    		if(valid_addresses[i] == ret_address)
		notFound = 0;
   
  	}  
 
	if(notFound){
		IMSG("Found invalid return address: %p whilst investigating FP %p", ret_address, fp);
		break;
	}
	...
	for(int i = 0; i < 880; i++) {
		...
		void* *current_address = stackptr + i;
		IMSG("  ; %p -> %p", current_address, *current_address);
        }
