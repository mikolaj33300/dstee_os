...
static void read_console(void)
{
	...
	IMSG("SP is now in handler uart iter: %x", spInSysMode);
		
		void* cpsrInSysMode;
		asm volatile ("mrs %0, cpsr\n\t" 
     : "=r" (cpsrInSysMode)
    );
    IMSG("CPSR is now in handler uart iter: %x", cpsrInSysMode);
...
static TEE_Result init_console_itr(void)
{
	...
	interrupt_set_affinity(console_itr.chip, console_itr.it, 0x1);
