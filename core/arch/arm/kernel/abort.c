...
void printr11(void) { 
    ...
    IMSG("r11val is: %x", r11val);
    
    	//for(int i = 0; i < 880; i++) {

		//void* *current_address = r11val + i;
		//if(0 != 0){
		//	IMSG("%x -> %x", r11val, *r11val);
		//}
		
        //}
}

void printPassedArg(uint32_t reg) { 
    void** r11val = reg;
    void* *valid_addresses[] = {0x117324, 0x117348, 0x1176a0, 0x117ac8}; 
    IMSG("reg passed is: %x", r11val);
    if(r11val > 0x142000 && r11val < 0x143000){
    	    
    	for(int i = 0; i < 1000; i++) {
		void* *current_address = r11val + i;
		if(current_address>=0x143000){
			break;
		}
		IMSG("%x -> %x", current_address, *current_address);
    	}
...
void print1(void) { IMSG("optee 1"); }
void print2(void) { IMSG("optee 2"); }
void print3(void) { IMSG("optee 3"); }
void print4(void) { IMSG("optee 4"); }
